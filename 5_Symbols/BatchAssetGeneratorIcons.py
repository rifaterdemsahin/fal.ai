...
def main():
    ...
    print(f"\n📊 Icons to generate: {len(GENERATION_QUEUE)}")
    
    # Count by priority
    high_priority = [a for a in GENERATION_QUEUE if a["priority"] == "HIGH"]
    medium_priority = [a for a in GENERATION_QUEUE if a["priority"] == "MEDIUM"]
    ...
    for i, asset in enumerate(GENERATION_QUEUE, 1):
        print(f"\n\n{'#'*60}")
        print(f"# Icon {i}/{len(GENERATION_QUEUE)}")
        print(f"{'#'*60}")
